...
// Keep the Mac awake only while sends are actually happening or about to. We
// deliberately do NOT hold it awake for jobs scheduled far in the future or
// paused for the daily cap - that would pin the machine on for hours/days for
// no reason (and future-scheduled sends need a scheduled wake anyway, since a
// sleeping Mac can't be caffeinated awake).
const KEEP_AWAKE_LOOKAHEAD_MS = 2 * 60 * 1000;

function hasActiveSendWork(jobs, now) {
  for (const job of jobs) {
    if (['completed', 'completed_with_errors', 'error'].includes(job.status)) continue;
    // A job currently being processed (including the 4-min pacing gaps between
    // its sends, during which its status stays 'sending').
    if (job.status === 'sending') return true;
    const pending = (job.recipients || []).filter((r) => r.status === 'pending');
    if (pending.length === 0) continue;
    if (job.schedulingMode === 'random') {
      // Stay awake for the whole open window so scattered sends aren't missed,
      // and for any recipient already due (e.g. a promoted fallback).
      if (job.window) {
        const start = new Date(job.window.start).getTime();
        const end = new Date(job.window.end).getTime();
        if (now >= start && now <= end) return true;
      }
      if (pending.some((r) => r.sendAt && new Date(r.sendAt).getTime() <= now + KEEP_AWAKE_LOOKAHEAD_MS)) {
        return true;
      }
    } else if (job.status === 'pending') {
      const due = job.scheduleAt ? new Date(job.scheduleAt).getTime() : now;
      if (due <= now + KEEP_AWAKE_LOOKAHEAD_MS) return true;
    }
  }
  return false;
}
...
function tick() {
  ...
  // Hold the Mac awake iff there's active/imminent sending work (no-op unless
  // on macOS with PREVENT_SLEEP_WHILE_PENDING enabled).
  keepAwake.update(hasActiveSendWork(jobs, now));
...
function start() {
  ...
  const cleanup = () => {
    ...
    keepAwake.release(); // don't leave a caffeinate process pinning the Mac awake
  };
