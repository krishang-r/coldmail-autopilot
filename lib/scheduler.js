...
async function runJob(job) {
  ...
  try {
    ...
    notify('Auto Email Sender - config error', err.message);
    store.updateJob(job.id, (j) => {
      j.status = 'error';
      j.error = err.message;
    });
  ...
  for (const recipient of job.recipients) {
    ...
    if (new Date(recipient.sendAt).getTime() <= now) {
      sendRandomOne(job.id, recipient.email).catch((err) => {
        ...
        notify('Auto Email Sender - job crashed', `${job.subject}: ${err.message}`);
      });
    }
  }
}
...
function tick() {
  ...
  try {
    ...
    notify('Auto Email Sender - scheduler error', err.message);
    return;
  }
  const now = Date.now();
